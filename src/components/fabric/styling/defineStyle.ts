...
type Classes<S extends Styles> = { [name in keyof S]: string };

type Return<S extends Styles> = (
  f: (classes: Classes<S>) => ReactNode,
) => ReactNode;

function defineStyle<S extends Styles>(
  getStyles: (theme: ITheme, props?: any) => S,
): Return<S>;
...
function defineStyle(arg: any): any {
  ...
  if (typeof arg == 'function') {
    const render = ({ props, theme, children }: any): any => {
      const classes = getClasses(
        (params: any) => arg(params.theme, params.props),
        { props, theme },
      );
