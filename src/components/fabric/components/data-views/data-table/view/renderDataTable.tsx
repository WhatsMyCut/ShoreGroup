...
function renderDataTable(props: DataTableProps, ref: any) {
  // TODO
  const [selectedRows, setSelectionRows] = React.useState(
      () => new Set<number>(),
    ),
    runOnUpdater = React.useRef([] as Function[]),
    rowSelectionMode =
      (props.rowSelectionOptions && props.rowSelectionOptions.mode) || 'none';
  ...
  function changeSelection(selection: Set<number>) {
    // TODO
  ...
  return styleDataTable(classes => {
    return (
      <div className={classes.container}>
        <AutoSizer>
          {({ width, height }) => {
            const columnWidths = calculateColumnWidths(props, width),
              dataColumns = props.columns.map((
                column: any,
                columnIndex: any, // TODO
              ) => (
                <Column
            ...
            if (
              ...
              props.rowSelectionOptions.mode !== 'none'
            ) {
              dataColumns.unshift(
                <Column
...
function calculateColumnWidths(props: DataTableProps, totalWidth: number) {
  ...
  const realTotal = totalWidth - selectorColumnWidth,
...
function createHeaderRow(
  ...
  // TODO
  const selectionMode = props.rowSelectionOptions
      ? props.rowSelectionOptions.mode
      : null,
    selectionColumn =
      selectionMode === 'none' ? null : (
        <div
          className={classes.rowSelectionColumn}
          style={{ minWidth: columnWidths.selectorColumn }}
        >
          <div>
            {selectionMode === 'multi'
  ...
  return (
    <div
      className={css('ReactVirtualized__Table__headerRow', classes.tableHead)}
    >
      {selectionColumn}
...
function createSelectRowCheckbox(
  rowIndex: number,
  props: DataTableProps,
  selectedRows: Set<number>,
  changeRowSelection: (selection: any) => void,
  classes: DataTableClasses,
) {
  // TODO
  const selectionMode = props.rowSelectionOptions
...
function createSelectAllCheckbox(
  ...
  // TODO
  const rowSelectionSize = selectedRows.size,
    checked = rowSelectionSize > 0 && rowSelectionSize === props.data.length,
    onChange = () => {
      const selectedRows: Iterable<number> = checked ? [] : props.data.keys();
  ...
  return (
    <Checkbox
      checked={checked}
      onChange={onChange}
      className={classes.selectAllRowsCheckBox}
    />
  );
}
