...
function renderDataExplorer(
  ...
  const dataTableRef = useRef((null as unknown) as DataTableMethods),
    onSortChange = useCallback((event: any) => {
      // TODO
      store.loadSorting(event.sortBy, event.sortDesc, props.loadData, () =>
        dataTableRef.current.unselectAllRows(),
      );
    }, []),
    onPageChange = useCallback((event: any) => {
      // TODO
      store.loadPage(event.pageIndex, props.loadData, () =>
        dataTableRef.current.unselectAllRows(),
      );
    }, []),
    onPageSizeChange = useCallback((event: any) => {
      // TODO
      store.loadPageSize(event.pageSize, props.loadData, () =>
        dataTableRef.current.unselectAllRows(),
      );
    }, []),
  ...
  const ret = styleDataExplorer(classes => {
    const loadingPanel = store.isLoading ? (
      ...
      <div className={classes.container}>
        ...
        <div className={classes.content}>
          <DataTable
            ...
            onRowSelectionChange={(event: RowSelectionChangeEvent) => {
        ...
        {store.pageIndex >= 0 &&
        store.pageSize > 0 &&
        (store.totalItemCount as number) > 0
...
function renderActionBar(
  ...
  props.actions.forEach((action, idx) => {
    ...
    */
    ...
    const hasIcon = !!action.icon,
      ...
      actionButtonClassName = disabled
  ...
  return (
    <CommandBar className={classes.actionBar} items={[]} farItems={items} />
  );
}
...
function renderFilterSections(
  ...
  const output: any = sections.map(section => {
    return section.contents.map(content => {
      const label = isBlankString(content.title) ? null : (
          <Label>{content.title}</Label>
        ),
        filters = (
          <DataExplorerFilterPanel filters={content.filters} store={store} />
        );
  ...
  return (
    <div className={classes.filterBox}>
      ...
      <div className={classes.filterSections}>{...output}</div>
