...
const specDefaultSearch = Spec.exact({
...
const specFilterSections = Spec.exact({
  type: Spec.is('sections'),
  sections: Spec.arrayOf(
    Spec.exact({
      type: Spec.is('section'),
      title: Spec.string,
      contents: Spec.arrayOf(
        Spec.exact({
...
const specTextFilter = Spec.exact({
  type: Spec.is('text'),
  name: Spec.match(REGEX_NAME),
  label: Spec.string,
});
