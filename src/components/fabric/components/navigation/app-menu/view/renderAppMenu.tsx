...
function renderWithCallout(props: AppMenuProps, classes: AppMenuClasses) {
  ...
  for (let i = 0; i < props.apps.length; ++i) {
    ...
    calloutContent.push(
      <div key={app.id} className={classes.appLink}>
        <div className={classes.appLinkTitle}>{app.title}</div>
        {app.description && (
          <div className={classes.appLinkDescription}>{app.description}</div>
        )}
      </div>,
    );
  }
  ...
  return (
    <div onClick={() => setCalloutVisible(true)}>
      <div ref={ref}>
        <label className={classes.label}>{props.apps[0].title}</label>
        <ChevronDownIcon />
      </div>
      <Callout
        target={ref.current}
