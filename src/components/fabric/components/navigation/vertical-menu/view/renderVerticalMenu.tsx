...
function VerticalMenuView(props: VerticalMenuProps) {
  ...
  if (props.items) {
    const linkGroups = getLinkGroups(props.items);

    ret = styleVerticalMenu(classes => {
      let onRenderGroupHeader:
        | IRenderFunction<INavLinkGroup>
        | undefined = undefined;

      if (
        props.collapsible === false ||
        (linkGroups.length === 1 && typeof linkGroups[0].name === 'string')
      ) {
        onRenderGroupHeader = ((linkGroup: INavLinkGroup) =>
          renderGroupHeader(linkGroup, classes)) as any;
      }
...
function getLinkGroups(items: any): INavLinkGroup[] {
  // TODO
...
function renderGroupHeader(
  linkGroup: INavLinkGroup,
  classes: VerticalMenuClasses,
) {
