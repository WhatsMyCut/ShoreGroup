...
function renderLoginForm(props: LoginFormProps, store: LoginFormStore) {
  let headerBox: ReactNode | null = null,
  ...
  return styleLoginForm(classes => {
    // TODO
    if (props.slotHeader) {
      headerBox = <div className={classes.header}>{props.slotHeader}</div>;
    } else {
      headerBox = (
    ...
    if (props.slotAbove) {
      aboveBox = <div className={classes.above}>{props.slotAbove}</div>;
    }

    if (props.slotBelow) {
      belowBox = <div className={classes.below}>{props.slotBelow}</div>;
    }

    const loginButtonText = store.isLoading() ? 'Logging in...' : 'Login',
    ...
    return (
      <div
        className={
          props.fullSize ? classes.containerFullSize : classes.container
        }
      >
          ...
          <div className={classes.card}>
            ...
            <form onSubmit={onSubmit} className={classes.form}>
              <div className={classes.content}>
                <div
                  className={
                    !props.extraFields || props.extraFields.length < 2
                      ? classes.fields
                      : classes.fieldsWithHorizontalLabel
                  }
                >
                  <div>
                    <div>
...
function LoginFormTextField(props: LoginFormTextFieldProps) {
  const value = props.store.getValue(props.field),
...
function LoginFormChoice(props: LoginFormChoiceProps) {
  const selectedKey = props.store.getValue(props.field),
    errorMsg =
