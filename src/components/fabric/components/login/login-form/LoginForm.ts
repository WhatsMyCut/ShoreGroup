...
const LoginForm = defineComponent<LoginFormProps>({
  ...
  validate: Spec.exactProps({
    optional: {
      performLogin: Spec.nullable(Spec.function),

      fullSize: Spec.boolean,

      extraFields: Spec.arrayOf(
        Spec.and(
      ...
      className: Spec.nullable(Spec.string),

      style: Spec.nullable(Spec.object),
  ...
  render(props) {
    ...
    if (props.extraFields) {
      props.extraFields.forEach(extraField => {
        initialValues[extraField.key] = extraField.hasOwnProperty(
          'defaultValue',
        )
