...
function renderBrand(props: BrandProps) {
  return styleBrand(classes => {
    const size = props.size,
      ...
      vendorClassName: string = css(
        classes.vendor,
        (classes as any)[`vendor${sizePascalCase}`],
      ),
      titleClassName: string = css(
        classes.title,
        (classes as any)[`title${sizePascalCase}`],
      ),
      logoClassName: string = css(
        classes.logo,
        (classes as any)[`logo${sizePascalCase}`],
      );

    let firstColumnContent = null,
    ...
    return (
      <div className={props.className} style={props.style}>
        <div className={classes.container}>
          <div className={classes.firstColumn}>{firstColumnContent}</div>
          <div className={classes.secondColumn}>{secondColumnContent}</div>
        </div>
      </div>
    );
  });
