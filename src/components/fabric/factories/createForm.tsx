...
type FormCtrl<T extends FormConfig> = {
  fields: { [name in keyof T['fields']]: FormFieldCtrl<T> };

  submit: () => void;
};

export default function createForm<T extends FormConfig>(
  config: T,
  f: (formCtrl: FormCtrl<T>) => ReactNode,
): void {}
...
type FormProps<T extends FormConfig> = {
  fields: { [name in keyof FormConfig['fields']]: FormFieldCtrl<T> };
  children: FormCtrl<T>;
};
